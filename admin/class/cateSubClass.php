...
class categorySub
{
    ...
    {
        $query = "INSERT INTO tbl_categorysub (category_id, categorySub_name)
                VALUES ('$category_id', '$categorySub_name')";
    ...
    {
        $query = "SELECT sub.*, cat.category_name 
                FROM tbl_categorysub AS sub
                LEFT JOIN tbl_category AS cat ON sub.category_id = cat.category_id
                ORDER BY sub.categorySub_id ASC";
        $result = $this->db->select($query);
        return $result;
    }
    public function update_categorySub($category_id, $categorySub_name, $categorySub_id)
    {
        $query = "UPDATE tbl_categorysub
                SET categorySub_name = '$categorySub_name', category_id = '$category_id'
                WHERE categorySub_id = '$categorySub_id'";
